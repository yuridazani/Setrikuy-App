...
const Reports = () => {
  ...
  const [summary, setSummary] = useState({ totalOmzet: 0, totalOrder: 0, totalPelanggan: 0 });
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const totalOmzet = orders.reduce((acc, o) => acc + o.total, 0);
      
      // 2. Siapkan Data Chart (7 Hari Terakhir)
    ...
    <div className="p-6 pb-32 animate-slide-up space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card className="p-4 bg-black text-white border-none shadow-xl">
          ...
          <h3 className="text-xl font-bold">{formatRupiah(summary.totalOmzet).split(',')[0]}</h3>
        </Card>
        <div className="space-y-3">
          <Card className="p-3 flex items-center gap-3">
            <div className="bg-orange/10 p-2 rounded-lg text-orange"><ShoppingBag size={16} /></div>
            <div>
              <p className="text-[10px] text-gray-400 font-bold uppercase">Transaksi</p>
              <p className="font-bold text-lg leading-none">{summary.totalOrder}</p>
            </div>
          </Card>
          <Card className="p-3 flex items-center gap-3">
            <div className="bg-primary/10 p-2 rounded-lg text-primary"><Users size={16} /></div>
            <div>
              <p className="text-[10px] text-gray-400 font-bold uppercase">Pelanggan</p>
              <p className="font-bold text-lg leading-none">{summary.totalPelanggan}</p>
            </div>
          </Card>
        </div>
      </div>
      ...
      <div className="bg-gray-100 p-4 rounded-2xl text-center text-xs text-gray-500">
        Data ini tersimpan lokal di perangkat Anda. <br/>
        Lakukan backup rutin di menu Pengaturan.
      </div>
    </div>
  );
};
